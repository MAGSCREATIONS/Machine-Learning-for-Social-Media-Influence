...
if "target" in df.columns and df["target"].nunique() > 1:
    ...
    print("Warning: target missing or single-class target; using non-stratified split.")
...
if "target" in df_train_std.columns and feature_columns:
    ...
    # Choose k using cross-validation on training data to avoid test leakage
    ...
    best_k_idx = int(np.argmax(cv_scores))
    best_k = k_values[best_k_idx]
    best_cv_accuracy = cv_scores[best_k_idx]
    print(f"Best k by 5-fold CV on training data: {best_k} ({best_cv_accuracy:.4f})")

    plt.figure(figsize=(8, 4))
    plt.plot(k_values, cv_scores, marker="o", linestyle="-")
    plt.title("KNN cross-validated accuracy by k")
